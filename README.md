...
FastAPI (JSON + agent status)
            |
            v
3D Dashboard (separate frontend)
```

Weather/validation stages реализованы внутри одного `ForecastAgent`, а не как отдельные LLM. Для работы не нужен API key или LLM.
...
**Timezone подтверждён пользователем: фиксированный UTC+05:00** для всего ряда. Не используется историческое изменение часового пояса `Asia/Almaty`. Внутри pipeline и во внешнем JSON — aware UTC; календарные признаки считаются в UTC+05:00.
...
Это автономный оркестратор с tools/state/recovery, **не LLM-агент** и не обучающийся планировщик. Внешний scheduler для самостоятельного периодического polling погоды в MVP не добавлен: новый расчёт инициируется запросом. Состояние в памяти рассчитано на **один uvicorn worker**; JSON результатов остаются на диске.
...
Секреты не требуются. `.env` исключён из Git. Оригинальные данные, модели, processed данные и weather cache включены в проект для воспроизводимого запуска. Если их убрали из своей копии, верните CSV с исходной русской схемой в `data/raw/turbine_1.csv` и `data/raw/turbine_2.csv`.
