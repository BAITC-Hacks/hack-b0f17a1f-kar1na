...
**Граница исходных данных:** оба CSV заканчиваются **31.01.2026 23:50 UTC+05:00**, что соответствует условию кейса, несмотря на февраль в названиях файлов. Февральские прогнозы рассчитаны, а февральские MAE/RMSE/R² равны `null`: фактические февральские значения участникам не предоставлены. Измеренное качество ниже относится к январскому holdout; отдельно проверен январский сценарий без обновления SCADA. Frontend включён в репозиторий и обслуживается тем же FastAPI после сборки.


## Быстрый запуск и проверка

Конфигурация Docker и CI использует Python 3.11 и Node.js 22. Локальная проверка выполнена на macOS с Python 3.9.6 и Node.js 25.3.0. Готовые модели и архив погоды уже включены.
...
Проверка без свежей SCADA без переобучения и сетевых запросов: `make check-scada`.
