...
Для просмотра графиков, 3D-модели и численного прогноза ключи не нужны. Чтобы пользоваться языковым ИИ-агентом, укажите свой `OPENAI_API_KEY` в `.env`: его запросы используют платный API. Ключ хранится только на сервере; `.env` исключён из Git. После изменения настроек перезапустите сервер.
