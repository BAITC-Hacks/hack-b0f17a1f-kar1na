...
export async function createTwin(onSelect){
 ...
 try{
 const scene=new THREE.Scene();scene.background=new THREE.Color('#132739');
 const renderer=new THREE.WebGLRenderer({antialias:true});renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.toneMapping=THREE.ACESFilmicToneMapping;host.append(renderer.domElement);
 ...
 document.querySelector('#camera-reset').onclick=()=>{farm.resetInspection();camera.position.set(-220,155,285);orbit.target.set(0,55,0);};
 ...
 return {select(id){selected=id;farm.selectTurbine(id);document.querySelector('#disassembly').value=farm.getExplodedAmount(id);},apply(id,p){farm.applyTurbineState(id,{windSpeed:p.wind_speed});},stop(){for(const id of ['turbine_1','turbine_2'])farm.setRotorSpeed(id,0);}};
