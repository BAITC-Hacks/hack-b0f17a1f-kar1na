...
export async function createTwin(onSelect){
 ...
 const gltf=await new GLTFLoader().loadAsync('/models/wind_farm.glb');scene.add(gltf.scene);weatherTurbine(gltf.scene);gltf.scene.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;}});const farm=createWindFarmControls(THREE,gltf);weatherTurbine(gltf.scene);let selected='turbine_1',zoomTarget=null;
 orbit.minDistance=8;
 ...
 gltf.scene.traverse(object=>{
   ...
   for(const material of [object.material].flat()){
     const weatherShader=material.onBeforeCompile;
     ...
     material.customProgramCacheKey=()=> 'windai-weathered-body-clip-v1';
 ...
 document.querySelector('#camera-reset').onclick=()=>{farm.resetInspection();farm.setNacelleExploded(selected,0,0);portrait(selected);document.querySelector('#disassembly').value=0;};
 document.querySelector('#inspect-reset').onclick=()=>{farm.resetInspection();farm.setNacelleExploded(selected,0,0);portrait(selected);document.querySelector('#disassembly').value=0;};
 document.querySelectorAll('[data-component]').forEach(b=>b.onclick=()=>{farm.setNacelleExploded(selected,0,0);farm.inspectComponent(selected,b.dataset.component);document.querySelector('#disassembly').value=0;focus(farm.getComponentFocusTarget(selected,b.dataset.component));});
